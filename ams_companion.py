...
def material_family(value: Any) -> str:
    """Normalize material labels enough to avoid mixing incompatible rolls."""
...
def normalized_rgb(value: Any) -> tuple[int, int, int] | None:
    """Recognize both 3MF hex colours and common labels from AMS RFID data."""
    labels = {
        "noir": (0, 0, 0), "blanc": (255, 255, 255), "gris": (128, 128, 128),
        "gris clair": (200, 200, 200), "rouge": (193, 46, 31), "orange": (245, 130, 32),
        "jaune": (254, 198, 0), "vert": (63, 142, 67), "cyan": (0, 180, 205),
        "bleu": (45, 105, 190), "violet": (128, 70, 170), "rose": (230, 90, 150),
    }
...
def color_distance(first: Any, second: Any) -> float | None:
    left, right = normalized_rgb(first), normalized_rgb(second)
...
def filament_signature(filament: dict[str, Any]) -> str:
    return f"{material_family(filament.get('type'))}|{str(filament.get('color') or '').upper()}"


def automatic_ams_mapping(
    filaments: list[dict[str, Any]], slots: dict[str, dict[str, Any]], learned: dict[str, str]
) -> tuple[dict[str, str], str]:
    """Match a sliced project to the rolls actually assigned to A1–A4.

    Only an unambiguous material/colour/RFID match is accepted.  A result is
    deliberately empty when two rolls remain plausible so Companion can ask
    the owner instead of deducting the wrong spool.
    """
    if not slots:
        return {}, "Aucune bobine AMS renseignée"
    chosen: dict[str, str] = {}
    ...
    for filament in filaments:
        ...
        candidates = [
            (slot, spool) for slot, spool in slots.items()
            if slot not in used_slots and family and material_family(spool.get("material")) == family
        ]
        if not candidates:
            return {}, f"Aucune bobine {family or 'compatible'} pour le filament {filament_id}"
        learned_tag = str(learned.get(filament_signature(filament), ""))
        learned_matches = [item for item in candidates if str(item[1].get("rfid_tag") or "") == learned_tag]
        if len(learned_matches) == 1:
            slot = learned_matches[0][0]
        elif len(candidates) == 1:
            slot = candidates[0][0]
        else:
            distances = sorted(
                (color_distance(filament.get("color"), spool.get("color")), slot)
                for slot, spool in candidates
            )
            if distances[0][0] is None or distances[0][0] > 70:
                return {}, f"Plusieurs bobines {family} sont possibles"
            runner = distances[1][0] if len(distances) > 1 else None
            if runner is not None and (runner is None or runner - distances[0][0] < 35):
                return {}, f"Plusieurs bobines {family} sont trop proches"
            slot = distances[0][1]
        chosen[filament_id] = slot
        used_slots.add(slot)
    return chosen, "Association matière/couleur/RFID sûre"
...
class Companion:
    ...
    def _remember_armed_mapping_locked(self) -> None:
        armed = self.state.get("armed_job") or {}
        learned = self.state["bridge"].setdefault("learned_mapping", {})
        installed = self.inventory.slot_spools()
        for line in armed.get("lines", []):
            spool = installed.get(str(line.get("slot"))) or {}
            tag = str(spool.get("rfid_tag") or "")
            if tag:
                learned[filament_signature(line.get("filament") or {})] = tag
    ...
    def _try_auto_arm_locked(self, force_fallback: bool = False) -> bool:
        ...
        self.mapping_proposal = None
        installed_slots = self.inventory.slot_spools()
        has_material_data = any(material_family(spool.get("material")) for spool in installed_slots.values())
        if (not requested_mapping and bridge.get("auto_match_enabled", True)
                and not force_fallback and has_material_data):
            proposed, reason = automatic_ams_mapping(
                filaments, installed_slots, bridge.get("learned_mapping", {})
            )
            self.mapping_proposal = {
                "mapping": proposed,
                "reason": reason,
                "plate": str(plate["id"]),
            ...
            if proposed:
                mapping = proposed
                mapping_source = "Association AMS automatique"
            else:
                if existing and existing.get("auto_bridge"):
                    self.state["armed_job"] = None
                changed = bridge.get("status") != f"Association AMS à confirmer — {reason}"
                bridge["mapping_confirmation_required"] = True
                bridge["mapping_conflict"] = []
                bridge["mapping_source"] = "Association AMS intelligente"
                bridge["status"] = f"Association AMS à confirmer — {reason}"
                if changed:
                    log(f"Passerelle: association AMS non certaine ({reason})")
                return changed
        ...
        self._remember_armed_mapping_locked()
