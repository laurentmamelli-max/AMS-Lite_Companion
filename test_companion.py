...
class CompanionTests(unittest.TestCase):
    ...
    def test_bridge_uses_unambiguous_material_and_colour_mapping(self):
        with tempfile.TemporaryDirectory() as tmp:
            app = ac.Companion(Path(tmp) / "state.json", [Path(tmp) / "watch"])
            app.update_inventory_spool(1, {"name": "PLA rouge", "material": "PLA", "color": "Rouge"})
            app.update_inventory_spool(2, {"name": "PLA bleu", "material": "PLA", "color": "Bleu"})
            app.configure_bridge({"default_mapping": {"1": "2"}})
            parsed = ac.parse_3mf(sample_3mf(12), "red.gcode.3mf")
            parsed["plates"][0]["filaments"][0]["color"] = "#C12E1F"
